...
static void AircraftEventHandler_Flying(Aircraft *v, const AirportFTAClass *apc)
{
	...
	if (CanVehicleUseStation(v, st) /*&& (st->owner == OWNER_NONE || st->owner == v->owner)*/ && !(st->airport.flags & AIRPORT_CLOSED_block)) //line commented for allow landing on guest airports
	{
