...
static bool CanEnterTileOwnerCheck(Owner owner, TileIndex tile, DiagDirection enterdir)
{
	if (IsTileType(tile, MP_RAILWAY) || // Rail tile (also rail depot)
		...
		return true;/*IsTileOwner(tile, owner);  // You need to own these tiles entirely to use them*/
	}
	...
	switch (GetTileType(tile)) {
		case MP_ROAD:
			...
			if (IsLevelCrossing(tile) &&
				...
				return true; /*IsTileOwner(tile, owner); // Railway needs owner check, while the street is public */
			}
			break;

		case MP_TUNNELBRIDGE:
			if (GetTunnelBridgeTransportType(tile) == TRANSPORT_RAIL) {
				return true;/*IsTileOwner(tile, owner);*/
			}
			break;
