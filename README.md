...
## Description
OpenTTD Experiment is a modified version of OpenTTD, where some game restrictions
are removed.

This version of OpenTTD is based on the 1.9.0-RC1 release.

This is an old project, now open-sourced, redocumented and archived.

## Changes
Changes are documented in [CHANGES.md](Changes/CHANGES.md).

The original files documenting the changes, [CHANGES.md](Changes/CHANGES.md) and 
the relevant visual demonstrations, including the [*savegame*](Changes/sample.sav) 
used for them, are located in the [*Changes*](Changes/) folder.

## History
I started this project because I like this game and, because it is open-source, 
I wanted to experiment with the game's code by enabling the game to do some things 
that in the original game i am not supposed to, like serving other companies' stations.

So I downloaded the source source of the latest version at the time (1.9.0-RC1) 
and started trying to make the wanted modifications.

After some trial and error, I was able to make some changes that worked out 
(but not bug free). Then I documented them, and played the game for a while for 
using the new features and finding new bugs.

Now I decided to open-source my work and enhance the documentation describing what I 
was able to make with simple changes to the game's logic.

During the documentation's enhancement, I found some bugs and other types of
issues that at the time I did not disover or note.
